...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (arg == "-tex" && argc > (i + 1)) {
            ...
            numberOfTextures++;
            Logger::Info("Adding texture: %s", argv[i + 1]);
        }
            ...
            Logger::Info("Start frame set to %d", startFrame);
        }
        ...
        settings.cubemapRes = atoi(argv[i + 1]);
            ...
            Settings::CaptureFormat f = [](std::string_view format) {
                if (format == "png" || format == "PNG") {
                    ...
                    Logger::Info("Unknown capturing format. Using PNG");
                    return Settings::CaptureFormat::PNG;
                }
            } (arg2);
            Settings::instance().setCaptureFormat(f);
            Logger::Info("Format set to %s", argv[i + 1]);
        }
            ...
            Logger::Info("Left path set to %s", argv[i + 1]);
        }
            ...
            Logger::Info("Right path set to %s", argv[i + 1]);
        }
    }
