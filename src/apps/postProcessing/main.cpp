...
void myPostSyncPreDrawFun()
{
	...
	if( gEngine->getWindowPtr()->isWindowResized() && !gEngine->isFisheye() )
		resizeFBOs();
...
void createFBOs()
{
	...
	{
		fb_width = gEngine->getWindowPtr()->getXFramebufferResolution();
		fb_height = gEngine->getWindowPtr()->getYFramebufferResolution();
	}
	...
	for(unsigned int i=0; i < thisNode->getNumberOfViewports(); i++)
	{	
		if( thisNode->getViewport(i)->isEnabled() && index < buffers.size())
		{
			buffers[index].fbo = 0;
			buffers[index].renderBuffer = 0;
			buffers[index].depthBuffer = 0;
			buffers[index].texture = 0;
			buffers[index].width = static_cast<int>(thisNode->getViewport(i)->getXSize() * static_cast<double>(fb_width));
			buffers[index].height = static_cast<int>(thisNode->getViewport(i)->getYSize() * static_cast<double>(fb_height));
			index++;
		}
	}
