...
#ifndef __SGCT__INTERNAL_FISHEYE_SHADERS_MODERN_CUBIC__H__
#define __SGCT__INTERNAL_FISHEYE_SHADERS_MODERN_CUBIC__H__

namespace sgct_core::shaders_modern_fisheye_cubic {
/*
...
constexpr const char* weightedMultisample_3f = R"(
    vec3 filter3f(vec2 texcoord, samplerCube map, vec4 bg) {
        ...
        vec3 sample1 = getCubeSample(
            texcoord + vec2(0.5, 0.5) / vec2(**size**, **size**),
            map,
            bg
        ).xyz;
        vec3 sample2 = getCubeSample(
            texcoord + vec2(-0.5, 0.5) / vec2(**size**, **size**),
            map,
            bg
        ).xyz;
        vec3 sample3 = getCubeSample(
            texcoord + vec2(-0.5, -0.5) / vec2(**size**, **size**),
            map,
            bg
        ).xyz;
        vec3 sample4 = getCubeSample(
            texcoord + vec2(0.5, -0.5) / vec2(**size**, **size**),
            map,
            bg
        ).xyz;
...
} // sgct_core::shaders_modern_fisheye_cubic

#endif // __SGCT__INTERNAL_FISHEYE_SHADERS_MODERN_CUBIC__H__
